...
| <a name="input_retention_bytes"></a> [retention\_bytes](#input\_retention\_bytes) | Length of messages to be retained in bytes. For standard plan retention bytes should be in the range [100 KiB, 1 GiB] and for enterprise plan it should be in the range [100 KiB, 2 TiB] | `list(number)` | `null` | no |
